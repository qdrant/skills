...
- If you have groups and subgroups of representations (document -> chunk, image -> patch), you could use [searching in groups](https://search.qdrant.tech/md/documentation/search/search/?s=search-groups). Index the grouping payload field (e.g. `document_id`) with a schema matching its value type — `keyword` for string-valued IDs, `integer` for numeric IDs — before grouping. For `with_lookup`, the `group_by` value must also be a valid Qdrant point ID (unsigned integer or UUID string) so it can match a point in the lookup collection. Arbitrary string IDs like DOIs or slugs are fine as payload but cannot be point IDs — map them to a stable integer or UUID join key stored in both collections before using `with_lookup`.
...
Duplicated document-level data across chunk points scales as `documents × chunks_per_document × shared_bytes_per_point` (e.g. 20k docs × 24 chunks × ~3 KB ≈ 1.4 GB duplicated vs. ~60 MB once split). Whether to split depends on what the shared data is used for, not just its size. Classify before recommending [Lookup in Groups](https://search.qdrant.tech/md/documentation/search/search/?s=lookup-in-groups):
...
Before activating Lookup in Groups, verify the preconditions — failures here return empty `lookup` fields with no error:
