...
- Use Qdrant for semantic memory: past conversations, learned facts, long-term context, and recall by meaning [Points](https://skills.qdrant.tech/md/documentation/manage-data/points/) [Payload](https://skills.qdrant.tech/md/documentation/manage-data/payload/)
...
- Search for similar memory before admitting a new point; admission should be "score before storing," not "always store" [Search](https://skills.qdrant.tech/md/documentation/search/search/) [Filtering](https://skills.qdrant.tech/md/documentation/search/filtering/)
- Update an existing point's payload (such as `updated_at`, provenance, counters, or freshness markers) when a highly similar memory already exists, rather than creating another near-duplicate point [Payload](https://skills.qdrant.tech/md/documentation/manage-data/payload/) [Points](https://skills.qdrant.tech/md/documentation/manage-data/points/)
...
- Treat similarity as the primary signal, then boost by recency and importance so retrieval reflects usefulness instead of pure embedding distance [Search relevance](https://skills.qdrant.tech/md/documentation/search/search-relevance/?s=score-boosting) [Search strategies](../qdrant-search-quality/search-strategies/SKILL.md)
- Store `created_at` or `updated_at` as a datetime payload field so recency can be applied during reranking rather than guessed in the prompt [Payload](https://skills.qdrant.tech/md/documentation/manage-data/payload/), [Payload Index](https://skills.qdrant.tech/md/documentation/manage-data/indexing/?s=payload-index)
- Use [Qdrant v1.14 or later score boosting](https://qdrant.tech/blog/qdrant-1.14.x/index.md#idea-2-reranking-most-recent-results) with a decay expression such as `gauss_decay` on the datetime field so fresher memories naturally rise without excluding older high-value memories [Search relevance](https://skills.qdrant.tech/md/documentation/search/search-relevance/?s=score-boosting)
...
- Every memory query must include a `must` filter on `agent_id`, `user_id`, `tenant_id`, or `session_id`; never run a naked semantic search across all memories [Filtering](https://skills.qdrant.tech/md/documentation/search/filtering/)
- Index the fields used on nearly every query path so isolation filters stay cheap at runtime [Payload Indexing](https://skills.qdrant.tech/md/documentation/manage-data/indexing/?s=payload-index)
...
- Check filter design next; if `session_id` or `agent_id` is in every query and not indexed, filtered retrieval becomes an avoidable bottleneck [Payload Index](https://skills.qdrant.tech/md/documentation/manage-data/indexing/?s=payload-index)
- Use scalar quantization when memory usage exceeds dataset expectations and recall remains acceptable; it is the default compression tradeoff for shrinking memory footprint without redesigning the system [Scalar Quantization](https://skills.qdrant.tech/md/documentation/manage-data/quantization/?s=scalar-quantization), [Memory usage optimization](../qdrant-performance-optimization/memory-usage-optimization/SKILL.md)
- Capacity planning must account for vectors, payload, indexes, and optimization headroom rather than raw embedding size alone [Capacity planning](https://skills.qdrant.tech/md/documentation/capacity-planning/)

## Preserve Provenance

Use when: the agent may need to explain, verify, update, or delete a memory later.

- Store `source` or `link_to_original` in the payload so each memory can be traced back to the originating conversation, tool output, or document [Payload](https://skills.qdrant.tech/md/documentation/manage-data/payload/)
