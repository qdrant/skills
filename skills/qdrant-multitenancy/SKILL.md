...
- Create a keyword payload index on the tenant field with `is_tenant=true` (the flag requires v1.11+). `is_tenant` tells Qdrant the field identifies tenants, so each tenant's vectors are stored together and served by sequential reads. 
...
## Few Non-Homogeneous Tenants (Collection per Tenant)

Use when: a limited number of tenants with different per-tenant embedding models or collection schemas.
...
- Creating too many collections may result in resource overhead and cause dependencies. This can increase costs and affect overall performance.
- It is not recommended to create hundreds and thousands of collections per cluster. It increases resource overhead, and can potentially lead to increased costs and at some point performance degradation and cluster instability. In Qdrant Cloud, the amount of collections per cluster is limited to 1000.
...
## What NOT to Do
- Create a collection per tenant by default. 
- Expect fast global, unfiltered queries after disabling the global index with `m=0` (per-tenant indexing makes cross-tenant scans slow by design)
